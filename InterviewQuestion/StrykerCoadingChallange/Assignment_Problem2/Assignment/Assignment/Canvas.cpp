...
void Canvas::drawEllipse(GLint cx, GLint cy, GLuint iWidth, GLuint iHeight, GLubyte red, GLubyte green, GLubyte blue)
{
	static const int numSlices = 72;
	static const GLfloat deltaAngle = 2.0f * std::atan2f(0.0f, -1.0f) / numSlices;
	cy = _height - cy;
	...
	GLfloat angle = 0.0f;
	for (int i = 0; i < numSlices; ++i, angle += deltaAngle) {
		glVertex2f(cx + 0.5f + sqrt(2) * iHeight * std::cosf(angle), cy + 0.5f + sqrt(2)* iWidth * std::sinf(angle));
	}
	glEnd();
}
